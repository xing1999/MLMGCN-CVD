...
from Modeling import NPZSeqDataset, SeqClassifier, QUANT_FEATURE_COLUMNS
...
def pad_2d_to_len(t: torch.Tensor, target_len: int, pad_value: float = 0.0):
    """t: [L, C] -> right-pad to [target_len, C] on CPU."""
    L, C = t.shape
    if L == target_len:
        return t
...
def pad_1d_to_len(t: torch.Tensor, target_len: int, pad_value: int = 0):
    """t: [L] -> right-pad to [target_len] on CPU."""
    L = t.shape[0]
    if L == target_len:
        return t
    pad_amount = target_len - L
...
# ------------------------- collate: variable length -> padded batch (CPU) -------------------------
def collate_and_pad_cpu(batch):
    """
  (sid, s_np, nt_np, shp_np, (idx_np, wts_np), nt_seq_np, quant_np, y)
    """
    sid_list = []
    s_list, nt_list, shp_list, ntseq_list, quant_list = [], [], [], [], []
    idx_list, wts_list, y_list = [], [], []

    # numpy -> CPU tensor
    for sid, s_np, nt_np, shp_np, (idx_np, wts_np), nt_seq_np, quant_np, y in batch:
        sid_list.append(sid)
        ...
        quant_list.append(torch.from_numpy(quant_np))   # [Cq]
    ...
    quant = torch.stack(quant_list, dim=0).float()      # [B, Cq]
    ...
    return sid_list, s, nt, shp, (idx, wts), ntseq, quant, y, mask
...
# ------------------------------------------------
def call_model(model, s, nt, shp, idx, wts, ntseq, quant=None, mask=None):
    sig = inspect.signature(model.forward)
    if "quant" in sig.parameters and "mask" in sig.parameters:
        return model(s, nt, shp, idx, wts, ntseq, quant=quant, mask=mask)
    elif "mask" in sig.parameters:
        return model(s, nt, shp, idx, wts, ntseq, mask=mask)
    else:
        return model(s, nt, shp, idx, wts, ntseq)

# ------------------------------------------------
def _to_device_batch(s, nt, shp, idx, wts, ntseq, quant, y, mask):
    ...
    quant = quant.to(DEVICE, non_blocking=True).float()
    ...
    return s, nt, shp, idx, wts, ntseq, quant, y, mask

def train_one_epoch(model, loader, optim, criterion):
    model.train()
    optim.zero_grad(set_to_none=True)
    total_loss, steps = 0.0, 0
    for _, s, nt, shp, (idx, wts), ntseq, quant, y, mask in loader:
        s, nt, shp, idx, wts, ntseq, quant, y, mask = _to_device_batch(s, nt, shp, idx, wts, ntseq, quant, y, mask)
        logit = call_model(model, s, nt, shp, idx, wts, ntseq, quant, mask).view(-1)  # [B]
...
def evaluate_find_thr(model, loader, criterion):
    ...
    for _, s, nt, shp, (idx, wts), ntseq, quant, y, mask in loader:
        s, nt, shp, idx, wts, ntseq, quant, y, mask = _to_device_batch(s, nt, shp, idx, wts, ntseq, quant, y, mask)
        logit = call_model(model, s, nt, shp, idx, wts, ntseq, quant, mask).view(-1)
        loss  = criterion(logit, y)
...
def build_model(sample_dim):
    if len(sample_dim) == 3:
        ...
        dim_quant = len(QUANT_FEATURE_COLUMNS)
    else:
        dim_s, dim_nt, dim_shape, dim_quant = sample_dim
    model = SeqClassifier(
        ...
        use_content_masked=USE_CONTENT_MASKED, num_queries=4, dim_quant=dim_quant
    ).to(DEVICE)
    return model
...
def main():
    ...
    # s/nt/shp [L, C]
    _, s0, nt0, shp0, _, ntseq0, quant0, _ = ds[0]
    dim_s   = s0.shape[-1]
    dim_nt  = nt0.shape[-1]
    dim_shp = shp0.shape[-1]
    dim_quant = quant0.shape[-1]
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(y_all)), y_all), start=1):
        ...
        model = build_model((dim_s, dim_nt, dim_shp, dim_quant))
