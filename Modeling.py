...
class NPZSeqDataset(Dataset):
    def __init__(self, labels_csv, feat_root, struct_mode="ss", use_nt_seq=True, quant_cols=None):
        assert struct_mode in ("ss", "shape"), "struct_mode only supports 'ss' or 'shape'"
        self.struct_mode = struct_mode
        self.use_nt_seq = use_nt_seq
        self.quant_cols = []

        df = pd.read_csv(labels_csv)
        assert "label" in df.columns, "labels.csv must contain 'label' column"
        self.labels = df["label"].astype(int).tolist()
        self.root = Path(feat_root)
        required_quant_cols = list(QUANT_FEATURE_COLUMNS if quant_cols is None else quant_cols)
        if quant_cols is None:
            extra_quant_cols = [
                c for c in df.columns
                if c.startswith("readmap_") and c not in {"label", "sequence"} and c not in required_quant_cols
            ]
            self.quant_cols = required_quant_cols + extra_quant_cols
        else:
            self.quant_cols = required_quant_cols
        missing_quant = [c for c in required_quant_cols if c not in df.columns]
        if missing_quant:
            preview = ", ".join(missing_quant[:5])
            if len(missing_quant) > 5:
                preview += ", ..."
            raise ValueError(
                f"labels_csv is missing required abundance/copy-number feature columns: {preview}. "
                "Please use data_readmap_abundance_copy_numeric.csv or keep these columns in the split labels.csv."
            )
        else:
            q = df[self.quant_cols].apply(pd.to_numeric, errors="coerce")
            q = q.replace([np.inf, -np.inf], np.nan).fillna(0.0)
            self.quant = q.to_numpy(dtype=np.float32, copy=True)
        self.dim_quant = int(self.quant.shape[1])

        if struct_mode == "ss":
            ss_dir = self.root / "ss_struct"
            if not ss_dir.exists():
                raise FileNotFoundError(f"Missing ss_struct directory: {ss_dir}")
        else:
            shp_dir = self.root / "dnashape"
            if not shp_dir.exists():
                raise FileNotFoundError(f"Missing DNAshape directory: {shp_dir}")
    ...
    def __getitem__(self, idx):
        ...
        quant = self.quant[idx].astype(np.float32, copy=False)
        y = int(self.labels[idx])
        return sid, s_emb, nt_emb, shp, (indices, weights), nt_seq, quant, y
...
class SeqClassifier(nn.Module):
    def __init__(self, nlayers, dim_s, dim_nt, dim_shape, hidden_dim, dropout, lamda, alpha, variant,
                 use_content_masked=True, num_queries=4, dim_quant=len(QUANT_FEATURE_COLUMNS)):
        super().__init__()
        self.dim_quant = int(dim_quant)
        ...
        self.quant_proj = nn.Sequential(
            nn.LayerNorm(self.dim_quant),
            nn.Linear(self.dim_quant, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden_dim * 3, 256), nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Linear(256, 64),             nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Linear(64, 1)
        )

    def _format_quant(self, quant, device):
        if quant is None:
            return torch.zeros(self.dim_quant, device=device, dtype=torch.float32)
        if not torch.is_tensor(quant):
            quant = torch.as_tensor(quant, device=device, dtype=torch.float32)
        else:
            quant = quant.to(device=device, dtype=torch.float32)
        quant = quant.reshape(-1)
        if quant.numel() != self.dim_quant:
            raise ValueError(f"Expected quant dim {self.dim_quant}, got {quant.numel()}")
        return torch.nan_to_num(quant, nan=0.0, posinf=0.0, neginf=0.0)


    # -------- single-sample forward --------
    def _forward_one(self, s_emb, nt_emb, shp, knn_indices, knn_weights, nt_seq, quant=None, shp_kpm=None):
        #  shape
        ...
        quant = self._format_quant(quant, g.device)
        z_quant = self.quant_proj(quant)   # [hidden]
        q_gate = self.quant_gate(quant)    # [hidden]
        g_fused = torch.cat([g * (1 + gate + q_gate), z_seq, z_quant], dim=-1)  # [3*hidden]

        return self.head(g_fused).squeeze(-1)


    def forward(self, s_emb, nt_emb, shp, knn_indices, knn_weights, nt_seq, quant=None, mask=None):
        if isinstance(quant, dict) and mask is None:
            mask = quant
            quant = None
        ...
        if s_emb.dim() == 2:
            ...
            if mask is not None and 'shp' in mask:
                ...
                if m.dim() == 1:
                    ...
                    shp_kpm = (m[0] == 0)
            return self._forward_one(s_emb, nt_emb, shp, knn_indices, knn_weights, nt_seq, quant, shp_kpm)
        ...
        for b in range(B):
            ...
            shp_b = shp[b]
            ntq_b = nt_seq[b]
            quant_b = None if quant is None else quant[b]
            idx_b = knn_indices[b]
            wts_b = knn_weights[b]
            ...
            logit_b = self._forward_one(s_b, nt_b, shp_b, idx_b, wts_b, ntq_b, quant_b, shp_kpm)
