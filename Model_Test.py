...
def evaluate(model, loader, thr=0.5):
    ...
    y_true, y_prob = [], []
    for batch in loader:
        for sid, s_np, nt_np, shp_np, (idx_np, wts_np), ntseq_np, quant_np, y in batch:
            ...
            quant = torch.from_numpy(quant_np).to(DEVICE)
            logit = model(s, nt, shp, idx_np, wts_np, ntseq, quant=quant)
...
def main():
    ...
    dim_quant = quant0.shape[-1] if hasattr(quant0, "shape") else len(QUANT_FEATURE_COLUMNS)
    ...
    for fold in range(1, 11):
        ...
        model = SeqClassifier(
            ...
            variant=False, use_content_masked=True, num_queries=4, dim_quant=dim_quant
        ).to(DEVICE)
    ...
    print("\n===== Test Results =====")
    print(df)
    print("Mean ACC: {:.3f} 卤 {:.3f}".format(df["ACC"].mean(), df["ACC"].std()))
    print("Mean AUC: {:.3f} 卤 {:.3f}".format(df["AUC"].mean(), df["AUC"].std()))
...
# from sklearn import metrics
#
# from Modeling import NPZSeqDataset, SeqClassifier  # adjust the import path if needed
#
# # ================== ==================
